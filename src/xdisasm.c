...
int fprintf_styled(void *f, enum disassembler_style style,
                        const char *fmt, ...) {
  va_list arg;

  va_start(arg, fmt);
  vsnprintf(currptr, MAX_INS_STRSIZE - (currptr - curr_insn_str), fmt, arg);
  currptr = curr_insn_str + strlen(curr_insn_str);
  return 0;
}
...
int init_dis_env(int arch, int bits, int endian) {
  ...
  case ARCH_arm:
    if (endian)
      disas = print_insn_big_arm;
    else
      disas = print_insn_little_arm;
    if (bits == 16)
      disas_options = "force-thumb";
    else if (bits == 64)
      disas = print_insn_aarch64;
    else
      disas_options = "no-force-thumb";
    break;
  case ARCH_mips:
    if (endian)
      disas = print_insn_big_mips;
    else
      disas = print_insn_little_mips;
    if (bits == 64)
      dis->mach = bfd_mach_mipsisa64;
    break;

  case ARCH_powerpc:
    if (endian)
      disas = print_insn_big_powerpc;
    else
      disas = print_insn_little_powerpc;
    if (bits == 64)
      disas_options = "64";
  ...
  case ARCH_x86:
    if (bits == 16)
      dis->mach = bfd_mach_i386_i8086;
    else if (bits == 64)
      dis->mach = !(bfd_mach_i386_i8086 | bfd_mach_i386_i386);
    else
  ...
  case ARCH_riscv:
    if (bits == 64)
      ...
      dis->mach = bfd_mach_riscv32;
    disas = print_insn_riscv;
    break;

  case ARCH_sparc: // TODO: use the different mach's based on the mach from the
                   // ELF
    dis->mach = bits;
    if (endian)
...
insn_t *disassemble_one(unsigned long long vma, char *rawbuf, size_t buflen,
  ...
  init_disassemble_info(dis, stdout, my_fprintf, fprintf_styled);
  buf = (bfd_byte *)rawbuf;
...
insn_list *disassemble(unsigned long long vma, char *rawbuf, size_t buflen,
                       int arch, int bits, int endian) {
